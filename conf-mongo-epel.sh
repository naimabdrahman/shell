...
#editing configuration of /etc/mongod.conf
#dont forget to replace bind_ip
#cat temp2.txt  | sed -e 's/#port = 27017/port = 27017/g' | sed -e 's/#keyFile = arg/keyFile = \/opt\/mongo\/mongo-keyfile/g' | sed -e 's/#replSet = arg/replSet = rs0/g' | sed -e 's/bind_ip = 127.0.0.1/bind_ip = 127.0.0.1,10.124.20.57/g'> temp3.txt
#cat temp3.txt > /etc/mongod.conf
